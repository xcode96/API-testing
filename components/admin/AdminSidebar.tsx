...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ onLogout, activeView, setActiveView }) => {
    const icons = {
    ...
    return (
        <aside className="w-64 bg-white/70 backdrop-blur-xl border-r border-slate-200 p-4 flex flex-col">
            ...
            <nav className="flex-1 space-y-2">
                 ...
                 <NavItem icon={icons.questions} label="Questions" active={activeView === 'questions'} onClick={() => setActiveView('questions')} />
                 <NavItem icon={icons.users} label="Users" active={activeView === 'users'} onClick={() => setActiveView('users')} />
                 <NavItem icon={icons.notifications} label="Notifications" active={activeView === 'notifications'} onClick={() => setActiveView('notifications')} />
                 <NavItem icon={icons.settings} label="Settings" active={activeView === 'settings'} onClick={() => setActiveView('settings')} />
            </nav>
